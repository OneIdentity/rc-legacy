...
int main(int argc, char **argv) {
    ...
    if( argc != 4 )
    {
        fprintf( stderr, "usage: %s <library to load> <username> <password>\n", argv[0] );
        exit( EINVAL );
    }
    else
    {
    ...
    */
    ...
    if( msgLen > 0 )
    {
        fprintf( stderr, "%s: ServerInit failed, error: <%s>\n", __FUNCTION__, errMsg );
        exit( 1 );
    }
    ...
    rval = fnsS.db2secValidatePassword( argv[2],
                                        strlen(argv[2]),
                                        NULL,
                                        0,
                                        0,
    ...
    if( rval != DB2SEC_PLUGIN_OK )
    {
        fprintf( stderr, "%s: ValidatePassword failed, error: <%s>\n", __FUNCTION__, errMsg ? errMsg : "(NULL)" );
        exit( 1 );
    }

    rval = fnsS.db2secGetAuthIDs( argv[2],
                                  strlen(argv[2]),
                                  "",
                                  0,
                                  0,
    ...
    if( rval != DB2SEC_PLUGIN_OK )
    {
        fprintf( stderr, "%s: GetAuthIDs failed, error: <%s>\n", __FUNCTION__, errMsg ? errMsg : "(NULL)" );
        exit( 1 );
    }

    if( !( !strncmp( argv[2], authID, strlen(argv[2]) ) && 
	       !strncmp( argv[2], sauthID, strlen(argv[2]) ) && 
	       !strncmp( argv[2], userid, strlen(argv[2])) ) )
    {
        fprintf( stderr, "%s: GetAuthIDs failed to get expected IDs, authID: <%s>, sauthID: <%s>, userid: <%s>\n", __FUNCTION__, authID ? authID : "(NULL)", sauthID ? sauthID : "(NULL)", userid ? userid : "(NULL)" );
        exit( 1 );
    }

    rval = fnsG.db2secGetGroupsForUser( argv[2], //authid
                                        strlen(argv[2]), //authidlen
                                        NULL, //userid
                                        0, //useridlen
                                        NULL, //usernamespace
                                        0, //usernamespacelen
    ...
    if( rval != DB2SEC_PLUGIN_OK )
    {
        fprintf( stderr, "%s: GetGroupsForUser failed, error: <%s>\n", __FUNCTION__, errMsg ? errMsg : "(NULL)" );
        exit( 1 );
    }
    ...
    fnsG.db2secFreeGroupListMemory( groupList, &errMsg, &msgLen );
