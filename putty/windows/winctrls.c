...
void editboxfw(struct ctlpos *cp, int password, char *text,
	       int staticid, int editid, int locked)
{
    RECT r;
...
void radioline(struct ctlpos *cp, char *text, int id, int nacross, ...)
{
    ...
    va_end(ap);
    radioline_common(cp, text, id, nacross, buttons, nbuttons, 0);
    sfree(buttons);
}
...
void bareradioline(struct ctlpos *cp, int nacross, ...)
{
    ...
    va_end(ap);
    radioline_common(cp, NULL, 0, nacross, buttons, nbuttons, 0);
    sfree(buttons);
}
...
void radiobig(struct ctlpos *cp, char *text, int id, ...)
{
    ...
    va_end(ap);
    radioline_common(cp, text, id, 1, buttons, nbuttons, 0);
    sfree(buttons);
}
 ...
 */
void checkbox_internal(struct ctlpos *cp, char *text, int id, int locked)
{
    RECT r;
...
void checkbox(struct ctlpos *cp, char *text, int id) {
    checkbox_internal(cp, text, id, 0);
}
 ...
 */
void staticbtn_internal(struct ctlpos *cp, char *stext, int sid,
	       char *btext, int bid, int locked)
{
...
void staticbtn(struct ctlpos *cp, char *stext, int sid,
	       char *btext, int bid)
{
    staticbtn_internal(cp, stext, sid, btext, bid, 0);
}

/*
 * A simple push button.
 */
void button(struct ctlpos *cp, char *btext, int bid, int defbtn, int locked)
{
    RECT r;
...
void staticedit_lk(struct ctlpos *cp, char *stext,
		int sid, int eid, int percentedit, int locked)
{
    staticedit_internal(cp, stext, sid, eid, percentedit, 
	    locked ? ES_READONLY : 0);
}

void staticpassedit_lk(struct ctlpos *cp, char *stext,
		    int sid, int eid, int percentedit, int locked)
{
    staticedit_internal(cp, stext, sid, eid, percentedit, 
	    (locked ? ES_READONLY : 0) | ES_PASSWORD);
}
...
void staticddl(struct ctlpos *cp, char *stext,
	       int sid, int lid, int percentlist, int locked)
{
...
void listbox(struct ctlpos *cp, char *stext,
	     int sid, int lid, int lines, int multi, int locked)
{
    RECT r;
...
void editbutton(struct ctlpos *cp, char *stext, int sid,
		int eid, char *btext, int bid, int locked)
{
		    ...
		    struct ctlpos *cp, struct controlset *s, int *id)
{
	...
	switch (ctrl->generic.type) {
	  ...
	  case CTRL_EDITBOX:
		...
		if (ctrl->editbox.has_list)
		    combobox(&pos, escaped,
			     base_id, base_id+1, ctrl->editbox.locked);
		else
		...
		if (ctrl->editbox.has_list) {
		    ...
		    (ctrl->editbox.password ? staticpassedit_lk : staticedit_lk)
	  ...
	  case CTRL_CHECKBOX:
	    ...
	    checkbox_internal(&pos, escaped, base_id, ctrl->checkbox.locked);
	    sfree(escaped);
	    break;
	  case CTRL_BUTTON:
	  ...
	  case CTRL_LISTBOX:
		...
		if (ctrl->listbox.percentwidth == 100) {
		    staticddlbig(&pos, escaped,
				 base_id, base_id+1, ctrl->listbox.locked);
		} else {
	  ...
	  case CTRL_FILESELECT:
	    ...
	    editbutton(&pos, escaped, base_id, base_id+1,
		       "Bro&wse...", base_id+2, ctrl->fileselect.locked);
	  ...
	  case CTRL_FONTSELECT:
	    ...
	    staticbtn_internal(&pos, "", base_id+1, "Change...", base_id+2,
		    ctrl->fontselect.locked);
	    sfree(escaped);
	    data = snew(FontSpec);
	    break;
	  default:
