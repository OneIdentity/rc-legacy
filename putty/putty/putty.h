...
struct config_tag {
    ...
    /* SSH options */
    /* Vintela change: increase command buffer size for long commands */
    char remote_cmd[20400];
    char *remote_cmd_ptr;	       /* might point to a larger command
				        * but never for loading/saving */
    char *remote_cmd_ptr2;	       /* might point to a larger command
				        * but never for loading/saving */
    double rate_limit;		       /* scp rate limit in bytes/second */
    int nopty;
    int compression;
...
/*
 * Policies are administrator-level preferences that override,
 * modify or preclude user preferences. PuTTY policies defined
 * here focus on network and security-related configuration.
 * Generally, if a value is -1, it indicates no policy is in place,
 * otherwise the value overrides any session preference. Policy
 * takes effect only when a session is loaded: it modifies the 
 * session after it is read.
 */
struct policy_tag {
    int no_remote_qtitle;
    int sshprot;
    int compression;
    int bad_ciphers[CIPHER_MAX];
    int agentfwd;
    int trust_dns_canon;
};
typedef struct policy_tag Policy;
extern Policy policy;
extern int no_policy;	/* -nopolicy flag */
