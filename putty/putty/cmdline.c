...
int cmdline_process_param(char *p, char *value, int need_save, Config *cfg)
{
    ...
    if (!strcmp(p, "-f")) {
        RETURN(1);
        cfg->sspi_fwd_ticket = 1;
    }
    if (!strcmp(p, "-k")) {
        RETURN(2);
        UNAVAILABLE_IN(TOOLTYPE_NONNETWORK);
        SAVEABLE(0);
        strcpy(cfg->service_principal_name, value);
    }
    if (!strcmp(p, "-c")) {
	char *name = get_username();
        if (!name) {
	    cmdline_error("couldn't get current username for -c");
            return 1;
        }
        strncpy(cfg->username, name, sizeof(cfg->username));
        sfree(name);
        RETURN(1);
    }
#endif
