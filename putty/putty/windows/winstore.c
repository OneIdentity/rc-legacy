...
void *open_settings_r(const char *sessionname)
{
	...
	/*
	 * If the user has not previously saved their own Default Settings,
	 * then try to read the ones stored in Group Policy.
	 */
	if (strcmp(sessionname, "Default Settings") != 0 ||
	    RegOpenKey(HKEY_CURRENT_USER, PUTTY_REG_POLICIES_DEFAULT_SETTINGS,
		    &sesskey) != ERROR_SUCCESS)
		sesskey = NULL;
    } else {
...
static int policy_opened;
static HKEY policy_key;

static int open_policy()
{
    if (!policy_opened) {
	policy_opened = 1;
	if (RegOpenKey(HKEY_CURRENT_USER, PUTTY_REG_POLICIES, &policy_key)
		!= ERROR_SUCCESS)
	    policy_key = 0;
    }
...
int read_policy_s(const char *key, char *buffer, int buflen)
{
    DWORD type, size;

    size = buflen;
    if (open_policy() &&
	    RegQueryValueEx(policy_key, key, 0, &type, 
...
int read_policy_i(const char *key, int *ret)
{
    DWORD type, val, size;
