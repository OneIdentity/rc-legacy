...
static void rate_limit(uint64 bytes, time_t start);
...
static void source(char *src)
{
	...
	rate_limit(stat_bytes, stat_starttime);
...
/* Introduces a delay if too many bytes have been sent */
static void rate_limit(uint64 bytes, time_t start)
{
    double rate;    /* in bytes/second */
    double sent;
    double diff;
    time_t now, wake;
    ...
    sent = uint64_to_double(bytes);
    if (sent < 1 || diff < 1)
	return;
    rate = sent / diff;

    if (rate > cfg.rate_limit) {
	wake = sent / cfg.rate_limit + start;
	delay = difftime(wake, now) * 1000;
	if (delay > 0)
	    Sleep(delay);  /* XXX Windows-specific */
    }
}
...
static void sink(char *targ, char *src)
{
	...
	while (uint64_compare(received,act.size) < 0) {
	    ...
	    rate_limit(received, stat_starttime);
	}
	if (act.settime) {
...
#ifdef SSPI_MECH
    printf("  -ng       skip sspi-based gssapi authentication\n");
    printf("  -k name   kerberos service principal name (forgo to use host name)\n");
#endif
    printf("  -sftp     force use of SFTP protocol\n");
    printf("  -scp      force use of SCP protocol\n");
    printf("  -limit r  maximum transfer rate in kiB/s\n");
#if 0
    /*
...
int psftp_main(int argc, char *argv[])
{
	...
	if (ret == -2) {
	    ...
	    if (!argv[i+1])
		cmdline_error("option -limit requires an argument");
	    cfg.rate_limit = atof(argv[i+1]) * 1024;
	    i++;
