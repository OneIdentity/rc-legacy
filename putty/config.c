			       ...
			       void *data, int event)
{
	...
	for (i = 0; i < CIPHER_MAX; i++) {
	    ...
	    int j;
	    char *cstr = NULL;
	    char disabledtext[128];
	    for (j = 0; j < (sizeof ciphers) / (sizeof ciphers[0]); j++) {
		if (ciphers[j].c == c) {
		    cstr = ciphers[j].s;
		    if (policy.bad_ciphers[c] == 1) {
			snprintf(disabledtext, sizeof disabledtext, 
			    "%s <disabled by policy>", cstr);
			cstr = disabledtext;
		    }
		    break;
		}
		      ...
		      int protocol, int protcfginfo)
{
    ...
    c->generic.locked = (policy.no_remote_qtitle != -1);
	...
	if (!midsession) {
		...
		char *str = dupprintf("Use system username (%s)",
			un ? un : "(null)");
		ctrl_text(s, "When username is not specified:", 
			HELPCTX(no_help));
		ctrl_radiobuttons(s, NULL, NO_SHORTCUT, 4,
			      HELPCTX(connection_username_fromenv),
			      dlg_stdradiobutton_handler,
			      I(offsetof(Config, username_from_env)),
			      "Prompt", NO_SHORTCUT, I(0),
			      str/*"Use system username"*/, NO_SHORTCUT, I(1),
			      NULL);
		sfree(str);
		sfree(un);
	    }
	...
	if (!midsession) {
	    ...
	    ctrl_radiobuttons(s, "Preferred SSH protocol version:", NO_SHORTCUT, 4,
			      ...
			      "2", '2', I(2),
			      "2 only", 'y', I(3), NULL);
	    c->generic.locked = (policy.sshprot != -1);
	}
...
#ifdef SSPI_MECH
	    ctrl_checkbox(s, "Enable GSSAPI algorithms", NO_SHORTCUT,
			  HELPCTX(ssh_kex_gss),
			  dlg_stdcheckbox_handler,
			  I(offsetof(Config,try_gsskex)));
#endif
...
#ifdef SSPI_MECH
	    ctrl_settitle(b, "Connection/SSH/GSSAPI",
			  "Options controlling GSSAPI authentication");

	    s = ctrl_getset(b, "Connection/SSH/GSSAPI", "main", NULL);

	    ctrl_checkbox(s, "Attempt GSSAPI auth (SSH-2)",
			  NO_SHORTCUT, HELPCTX(ssh_auth_gssapi),
			  dlg_stdcheckbox_handler,
			  I(offsetof(Config,try_sspi_auth)));

	    ctrl_checkbox(s, "Server determines username from credentials",
			  NO_SHORTCUT,
			  HELPCTX(ssh_gssapi_server_determines_username),
			  dlg_stdcheckbox_handler,
			  I(offsetof(Config,sspi_no_username)));
	    ctrl_checkbox(s, "Delegate credentials",
			  NO_SHORTCUT, HELPCTX(ssh_auth_gssapi_fwd_ticket),
			  dlg_stdcheckbox_handler,
			  I(offsetof(Config,sspi_fwd_ticket)));
	    c = ctrl_checkbox(s, "Trust DNS",
			  NO_SHORTCUT, HELPCTX(ssh_trust_dns),
			  dlg_stdcheckbox_handler,
			  I(offsetof(Config,sspi_trust_dns)));
	    c->generic.locked = (policy.trust_dns_canon == 0);
	    ctrl_editbox(s, "Service Principal Name: (Kerberos)",
			  NO_SHORTCUT, 100, HELPCTX(ssh_auth_gssapi_prin_name),
			  dlg_stdeditbox_handler,
			  I(offsetof(Config,service_principal_name)),
			  I(sizeof(((Config *)0)->service_principal_name)));
#endif
	}
