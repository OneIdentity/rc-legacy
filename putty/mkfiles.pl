...
if (defined $makefiles{'cygwin'}) {
    ...
    "\n".
    &splitline("CFLAGS = -mno-cygwin -Wall -O2 -D_WINDOWS -DWINVER=$WINVER ".
#      "-DWIN32S_COMPAT ".  # cygwin-specific
#      "-D_NO_OLDNAMES ".   # cygwin-specific
#      "-DNO_MULTIMON ". 
#      "-DNO_HTMLHELP ". 
	       (join " ", map { my $p = "\$(srcdir)$dirpfx$_"; 
		                $p =~ s,^(\./)+,,;   # reqd for MinGW 4.1.0
		                $p =~ s,(/|/\.)*/$,/,; 
				"-I$p"
			  } @srcdirs)) .
	       "\n".
    "LDFLAGS = -mno-cygwin -s\n".
    &splitline("RCFLAGS = -I\$(srcdir). \$(RCINC) --define WIN32=1 --define _WIN32=1".
    ...
    foreach $d (&deps("X.o", "X.res.o", "\$(srcdir)".$dirpfx, "/", "cygwin")) {
...
if (defined $makefiles{'borland'}) {
    ...
    "\n".
    "# C compilation flags\n".
    "CFLAGS = -D_WINDOWS -DWINVER=$WINVER\n".
    "# Resource compilation flags\n".
    "RCFLAGS = -DNO_WINRESRC_H -DWIN32 -D_WIN32 -DWINVER=$WINVER\n".
    "\n".
...
if (defined $makefiles{'vc'}) {
    ...
    print
      ...
      "CFLAGS = /nologo /W3 /O1 " .
      (join " ", map {"-I$dirpfx$_"} @srcdirs) .
      " /D_WINDOWS /D_WIN32_WINDOWS=$WINVER /DWINVER=$WINVER\n".
